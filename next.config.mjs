...
const nextConfig = {
  ...
  async redirects() {
    return [
      ...
      // its opengraph-image/embed-preview children, and query strings.
      // Evaluated before the filesystem, so nothing can shadow it. In-app
      // code must build /artwork links directly (enforced by
      // no-restricted-syntax in eslint.config.mjs); this rule is the only
      // sanctioned /moment path.
      {

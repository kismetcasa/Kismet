...
export function BuyButton({ listing, onBought, className = '', compact = false }: BuyButtonProps) {
  ...
  return (
    <button
      ...
      // Same treatment as the collect buttons: gradient letters (on an inner
      // span — see MomentCard.renderCollectButton for the Chromium
      // background-clip:text seam this avoids), plain box, gradient fill on
      // hover — the label carries the price, so it stays visible in the
      // action. `bought` is NOT a differentiated accent box, mirroring the
      // un-differentiated collect+.
    ...
    >
      <span className="accent-grad">{bought ? 'bought' : loading ? 'buying…' : `buy ${priceLabel}`}</span>
    </button>
  )
}

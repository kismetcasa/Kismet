...
| `kismetart:fc:push-fids` | set fid | ▶ SADD in registerToken MULTI; SREM in clearTokens MULTI / erase / lazy prune during broadcast enumeration; one-time backfill (`scripts/reconcile-fc-push-fids.mjs`) | SSCAN count 1000 per broadcast run (`collectBroadcastTargets`) | none (self-healing membership; 100k scan cap, logged) | O (missing member = user silently unreached by broadcasts) |
| `kismetart:fc:broadcast:{notificationId}` | string JSON stats | SET EX 30d per run (`broadcastFarcasterPush`) | GET via `/api/admin/broadcast?notificationId=` | 30d | O (stats advisory; run logged regardless) |
| `kismetart:fc:broadcast-lock` | string uuid | SET NX EX 600 + Lua release (redisLock) | — | 600s | O (concurrent runs harmless — host-side dedupe) |

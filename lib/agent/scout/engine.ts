...
export interface ScoutPolicy {
  ...
  /** How many editions of each drop to hold before the agent stops collecting it
   *  (>= 1; default 1 = "one of each new drop"). Enforced on-chain at execution
   *  (skip when balanceOf >= this), so it covers open editions with no index lag. */

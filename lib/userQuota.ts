...
const QUOTAS: Record<QuotaKind, QuotaWindow> = {
  ...
  // Collector-file versions (app/api/collector-file). Each is a PERMANENT
  // ciphertext on Arweave (~$0.51 worst case at the 16 MiB cap), so counts
  // and bytes get their OWN meters — debiting 'upload-bytes' would let 15
  // zip iterations lock an artist out of minting metadata for the day.
  // These fail open like every kind here; the fail-CLOSED backstop on
  // aggregate spend is consumeCfilePlatformBytes (lib/collectorFile).
  'cfile-upload':   { day: 15,                 week: 60                  },
  'cfile-bytes':    { day: 256 * 1024 * 1024,  week: 1024 * 1024 * 1024  },
  // Decrypt-and-send downloads: bounds a holder's session being farmed as a
  // free CDN; far above any human re-download cadence.

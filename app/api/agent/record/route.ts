 ...
 * on the receipt; idempotent) and PATCH /api/listings/{id} (verifies the Seaport
 * OrderFulfilled on the receipt; 409 once filled). A forged or replayed URL can
 * therefore only ever record a mint or sale that happened. The delegation is an
 * in-process call carrying the caller's headers, so rate limits still key on
 * the real client.
 *
...
export async function GET(req: NextRequest) {
  ...
  if (verb === 'collect') {
    ...
    const currency = q.get('currency') ?? 'eth'
    if (currency !== 'eth' && currency !== 'usdc') return errorResponse(400, 'currency must be "eth" or "usdc"')
    const pricePerToken = q.get('pricePerToken') ?? '0'
    if (!/^[0-9]+$/.test(pricePerToken)) return errorResponse(400, 'pricePerToken must be base units (digits)')
...
/** Invoke a record handler in-process with a request that carries the caller's
 *  headers (IP attribution, cookies) and the JSON body the app would post. */

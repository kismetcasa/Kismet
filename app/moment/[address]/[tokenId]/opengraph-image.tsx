...
import { readPreferredStill } from '@/lib/media/animatedPreview'
import {
  shareCard,
  SHARE_CARD_SIZE,
...
export default async function Image({ params }: Props) {
  ...
  if (isAddress(address) && isValidTokenId(tokenId)) {
    ...
    if (detail && !detail.hidden) {
      ...
      // Video/gif moment whose poster the warm-time marker judged flat-black
      // (or absent, or the video itself): prefer the representative still
      // extracted from the video at a VERIFIED non-black offset (cached
      // alongside the animated preview). The marker is decided from the actual
      // poster bytes during the background warm — never inline here — so this
      // stays a pure cache read on the crawler path. Byte-identical fallback:
      // cold cache or a real (non-black) creator cover → imageUrl stays
      // exactly as shareImageSource resolved it. readPreferredStill is a no-op
      // (null) for non-video/gif metadata; the label gate just skips the reads
      // for stills/writing.
      if (label === 'VIDEO') {
        const still = await readPreferredStill(detail.metadata)

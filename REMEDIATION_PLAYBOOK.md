...
6. **Transitive vulns: bump the _parent_, not the leaf.** For the turbo-sdk→arbundles
   and CDP→axios chains the fix lives in the parent package — upgrade the parent
   (turbo-sdk / CDP) so the leaf resolves, or pin via root `overrides`; verify with
   `npm ci` + build. (Done manually: an automated update bot was tried and removed —
   the auto-PR branch churn was unwanted.)
...
- Gate CI on **`npm audit --audit-level=high`** (block the 3 critical / 8 high; report
  ...
  (turbo-sdk / CDP — that's where the fix lives) or via root-level **`overrides`**,
  preferring **nested/`$`-scoped**
...
| 67 npm vulns (3 crit / 8 high) | OWASP **A06 Vulnerable & Outdated Components** | SCA in CI; parent-bump / `overrides`; remove unused; patch cadence |
...
3. **CI** (`npm run check` + `npm audit --audit-level=high` + build, least-priv token, branch protection); schedule the critical arbundles/turbo-sdk + axios/CDP upgrades manually (parent-bump / `overrides`). ([GH Actions](https://docs.github.com/en/actions/tutorials/build-and-test-code/nodejs), [overrides](https://docs.npmjs.com/cli/v11/configuring-npm/package-json/))
